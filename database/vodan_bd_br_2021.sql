...
#========================================================================================================================
 ...
 select crfformsid as modId, questionid as qstId, questionorder as qstOrder, qstgroupid as qstGroupId,
		  ...
		  translate('pt-br', question) as dsc_qst,
                ...
                (Select description from tb_listtype where listTypeID = tb_Questionario.listtypeid) as qst_list_type,
	            ...
	            (select  GROUP_CONCAT(convert(tlv1.listOfValuesID, char)) as listofvalueID from tb_listofvalues tlv1
                 ...
                 (Select comment from tb_listtype where listTypeID = tb_Questionario.listtypeid) as qst_list_type_comment
		from (
			select crfformsid, questionid, questioncomment, questionorder, 
				   formulario,
			...
			select t5.crfformsid, t1.questionid, t1.comment as questioncomment, t9.questionorder,
                  ...
                  (select t2.questionGroupID from tb_questiongroup t2 where t2.questiongroupid = t1.questiongroupid) as qstgroupid,
				   (select t2.comment from tb_questiongroup t2 where t2.questiongroupid = t1.questiongroupid) as questionGroupComment,
                 ...
                 (select t3.comment as questionTypeComment from tb_questiontype t3 where t3.questiontypeid = t1.questiontypeid) as questionTypeComment,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getqst_rsp_modulo_medicalRecord` (IN `p_formRecordId` INT)  BEGIN
#========================================================================================================================
...
sp:BEGIN 
   ...
   START TRANSACTION;
    ...
    INSERT INTO tb_notificationrecord (
			userid, profileid, hospitalunitid, tablename, rowdid, changedon, operation, log)
            values (p_userid, p_grouproleid, p_hospitalunitid, 'tb_questionnaire', 1, now(), 'I', CONCAT('Criação de pesquisa baseada: ', p_questionnaireDescription));
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `putQstChangeOrder` (IN `p_moduleID` INT, IN `p_questionsorder` TEXT, OUT `p_msg_retorno` VARCHAR(500))  BEGIN
#========================================================================================================================
#== Procedure criada para alterar as ordens das questões em um determinado módulo
#== Criada em 17 de março de 2022
#== A entrada para esta procedure é uma string que contém os valores novos de ordem no seguinte formato:
#== A entrada apra esta procedure é uma string do tipo: id_1:value_1,id_2:value_2,..id_n:value_n
#== Onde id_i: id da pergunta e value_i é a nova ordem da questão
#========================================================================================================================
   ...
   START TRANSACTION;
    ...
    while length(v_lista) > 1 DO
        ...
        if v_questionid is not null then
			...
			update tb_questiongroupform set questionOrder = CONVERT(v_answer,UNSIGNED) where questionID = v_questionid and questionID = (Select questionID from tb_questiongroupform where crfFormsID = p_moduleID and questionID = v_questionid);
            set p_msg_retorno = 'Ordem das questões atualizadas com sucesso.';
        end if;
...
#== Criada em 21 dez 2020
#== Alterada em 24 jan 2021 
#== A entrada apra esta procedure é uma string do tipo: id_1:value_1,id_2:value_2,..id_n:value_n
#========================================================================================================================

DECLARE v_Exist integer; # Está sendo usado para checar se existe entrada em pt-br na multilanguage
DECLARE v_question varchar(50);
DECLARE v_answer varchar(500); # variavel auxiliar da nova descrição do grupo de questão
DECLARE v_questionid integer; # variavel auxiliar do id do grupo de questão
DECLARE v_listOfValuesid integer;
DECLARE v_listtypeid integer;
DECLARE v_questionGroupFormid integer;
DECLARE v_lista text;
...
sp:BEGIN		
   START TRANSACTION;
    ...
    set v_lista = concat(p_questionsgroupsdescriptions, ',') ;
    ...
    while length(v_lista) > 1 DO
        ...
        if v_questionid is not null then
            ...
            set v_Exist = (Select description from tb_multilanguage where description = v_answer and languageID = 1);
              if v_Exist is null THEN
              	insert into tb_multilanguage(languageID, description, descriptionLang) VALUES(1,v_answer,v_answer);
              end if;
              
			Update tb_questiongroup set description = v_answer where questionGroupID = v_questionid and questionGroupID IN (Select questionGroupID from tb_questions where questionID IN (Select questionID from tb_questiongroupform where crfFormsID = p_moduleID));
            set p_msg_retorno = 'Textos das descrições dos agrupamentos alterados com sucesso.';
        end if;
...
DECLARE v_Exist integer; # Está sendo usado para checar se existe entrada em pt-br na multilanguage
DECLARE v_question varchar(50);
DECLARE v_answer varchar(500); # variavel auxiliar da nova descrição da pergunta
DECLARE v_questionid integer; # variavel auxiliar do id da pergunta que será alterada
DECLARE v_listOfValuesid integer;
DECLARE v_listtypeid integer;
DECLARE v_questionGroupFormid integer;
...
sp:BEGIN		
   START TRANSACTION;
    ...
    set v_lista = concat(p_questionsdescriptions, ',') ;
    ...
    while length(v_lista) > 1 DO
        ...
        if v_questionid is not null then
			...
			update tb_questions set description = v_answer where questionID = v_questionid and questionID = (Select questionID from tb_questiongroupform where crfFormsID = p_moduleID and questionID = v_questionid);
  ...
  #      SELECT v_lista, length(v_lista), position(',' in v_lista);
    ...
    set p_msg_retorno = 'Textos das questões alterados com sucesso.';
...
(585, 32, 'Discharge/death form', 2, '2022-01-09 19:34:04', '2022-01-09 19:34:04'),
(586, 33, 'Discharge/death form', 2, '2022-01-10 02:29:12', '2022-01-10 02:29:12'),
(587, 34, 'Follow-up', 2, '2022-01-10 02:31:31', '2022-01-10 02:31:31'),
(588, 35, 'Admission form', 2, '2022-01-10 19:13:57', '2022-01-10 19:13:57'),
(589, 35, 'Follow-up', 2, '2022-01-10 19:13:57', '2022-01-10 19:13:57'),
...
(1, 'Antiviral list', 'Este é um comentário sobre o tipo da lista'),
(2, 'AVPU list', 'Este é um comentário sobre o tipo da lista'),
(3, 'Coronavirus list', 'Este é um comentário sobre o tipo da lista'),
(4, 'Corticosteroid list', 'Este é um comentário sobre o tipo da lista'),
(5, 'Country list', 'Este é um comentário sobre o tipo da lista'),
(6, 'HIV list', 'Este é um comentário sobre o tipo da lista'),
(7, 'Interface list', 'Este é um comentário sobre o tipo da lista'),
(8, 'O2 flow list', 'Este é um comentário sobre o tipo da lista'),
(9, 'Outcome list', 'Este é um comentário sobre o tipo da lista'),
(10, 'Outcome saturation list', 'Este é um comentário sobre o tipo da lista'),
(11, 'pnnotdone_list', 'Este é um comentário sobre o tipo da lista'),
(12, 'self_care_list', 'Este é um comentário sobre o tipo da lista'),
(13, 'sex at birth list', 'Este é um comentário sobre o tipo da lista'),
...
(1, 'Nomin da Instalaução', 'Nomin da Instalaução'),
(1, 'Grávida?', 'Grávida?'),
(1, 'Nome da instalação ??', 'Nome da instalação ??'),
(1, 'Nome da instalaçãum', 'Nome da instalaçãum'),
(1, 'Nome da instalaçãum 2', 'Nome da instalaçãum 2'),
(1, 'Nome da instalaçãum 3', 'Nome da instalaçãum 3'),
(1, 'Nome da instalaçãum 4', 'Nome da instalaçãum 4'),
(1, 'Nome da instalaçãum 5', 'Nome da instalaçãum 5'),
(1, 'Nome da instalaçãum 6', 'Nome da instalaçãum 6'),
(1, 'Nome da instalaçãum 7', 'Nome da instalaçãum 7'),
(1, 'Nome da instalaçãum 8', 'Nome da instalaçãum 8'),
(1, 'Nome da instalaçãum 9', 'Nome da instalaçãum 9'),
(1, 'Nome da instalaçãum 10', 'Nome da instalaçãum 10'),
(1, 'Nome da instalaçãum 11', 'Nome da instalaçãum 11'),
(1, 'Medicatation', 'Medicatation'),
(1, 'Renekaton', 'Renekaton'),
(1, 'Vacination', 'Vacination'),
(1, 'Localidade', 'Localidade'),
(1, 'Paíse', 'Paíse'),
(1, 'Nome da Instalaçãoo', 'Nome da Instalaçãoo'),
(1, 'TExto', 'TExto'),
(1, 'Medicados', 'Medicados'),
(1, 'Nome da Instalasção', 'Nome da Instalasção'),
(1, 'Dados demográficossd', 'Dados demográficossd'),
...
(20, 1, 1, 'tb_participant', 187, '2022-02-11 21:31:35', 'I', 'Inclusão de paciente: 54654654'),
(20, 1, 1, 'tb_assessmentquestionnaire', 0, '2022-02-11 21:31:35', 'I', 'Inclusão do registro referente ao paciente: 187 para o hospital: 1'),
(3, 7, 1, 'tb_participant', 168, '2020-12-08 18:19:34', 'I', 'Inclusão de paciente: 808080808081'),
(3, 7, 1, 'tb_participant', 169, '2020-12-08 18:22:27', 'I', 'Inclusão de paciente: 808080808082'),
(3, 7, 1, 'tb_assessmentquestionnaire', 0, '2020-12-08 18:22:27', 'I', 'Inclusão do registro referente ao paciente: 169 para o hospital: 1'),
(3, 7, 1, 'tb_formRecord', 238, '2020-12-08 18:29:18', 'I', 'Inclusão de Modulo para paciente: 168'),
(3, 7, 1, 'tb_questiongroupformrecord', 1129, '2020-12-08 18:29:18', 'I', 'Inclusão da questão referente a data do Modulo para paciente: 168'),
(3, 1, 1, 'tb_user', 8, '2020-12-09 21:43:06', 'I', 'Inclusão de dados do Usuario: Admin HUGG'),
(8, 1, 1, 'tb_userrole', 0, '2020-12-09 21:57:22', 'A', 'Suspenso acesso do usuário5 ao hospital Hospital Universitário Gaffrée e Guinle - HUGG'),
(8, 1, 1, 'tb_userrole', 0, '2020-12-09 21:57:42', 'A', 'Retorno de acesso do usuário5 ao hospital Hospital Universitário Gaffrée e Guinle - HUGG'),
(8, 1, 1, 'tb_user', 9, '2020-12-09 22:05:04', 'I', 'Inclusão de dados do Usuario: Teste5'),
(3, 7, 1, 'tb_participant', 0, '2020-12-21 18:22:39', 'A', 'Alteração de  Prontuario de: 100011583533 para 100011589999'),
(3, 7, 1, 'tb_participant', 0, '2020-12-21 18:27:30', 'A', 'Alteração do paciente 0 - Prontuario de: 100011589999 para 100011583533'),
(3, 7, 1, 'tb_participant', 0, '2020-12-21 18:29:35', 'A', 'Alteração - Prontuario de: 100011583533 para 100011589999'),
(3, 7, 1, 'tb_participant', 0, '2020-12-21 18:29:47', 'A', 'Alteração - Prontuario de: 100011589999 para 100011583533'),
(3, 7, 1, 'tb_participant', 88, '2020-12-21 18:31:25', 'A', 'Alteração - Prontuario de: 100011583533 para 100011589999'),
(3, 7, 1, 'tb_participant', 88, '2020-12-21 18:31:46', 'A', 'Alteração - Prontuario de: 100011589999 para 100011583533'),
(3, 7, 1, 'tb_participant', 170, '2020-12-22 15:37:45', 'I', 'Inclusão de paciente: 808080808888'),
(3, 7, 1, 'tb_assessmentquestionnaire', 0, '2020-12-22 15:37:45', 'I', 'Inclusão do registro referente ao paciente: 170 para o hospital: 1'),
(3, 7, 1, 'tb_participant', 171, '2020-12-22 15:55:21', 'I', 'Inclusão de paciente: 808080808889'),
(3, 7, 1, 'tb_assessmentquestionnaire', 0, '2020-12-22 15:55:21', 'I', 'Inclusão do registro referente ao paciente: 171 para o hospital: 1'),
(3, 7, 1, 'tb_participant', 171, '2020-12-22 16:12:22', 'A', 'Alteração - Prontuario de: 808080808889 para 808080808890'),
(3, 7, 1, 'tb_participant', 171, '2020-12-22 16:12:44', 'A', 'Alteração - Prontuario de: 808080808890 para 808080808889'),
(3, 7, 1, 'tb_participant', 172, '2020-12-22 17:05:49', 'I', 'Inclusão de paciente: 808080808891'),
(3, 7, 1, 'tb_assessmentquestionnaire', 0, '2020-12-22 17:05:49', 'I', 'Inclusão do registro referente ao paciente: 172 para o hospital: 1'),
(11, 1, 1, 'tb_participant', 173, '2021-01-25 18:03:48', 'I', 'Inclusão de paciente: 123456'),
(11, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-01-25 18:03:48', 'I', 'Inclusão do registro referente ao paciente: 173 para o hospital: 1'),
(11, 1, 1, 'tb_participant', 174, '2021-01-25 18:07:47', 'I', 'Inclusão de paciente: 1234567'),
(11, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-01-25 18:07:47', 'I', 'Inclusão do registro referente ao paciente: 174 para o hospital: 1'),
(11, 1, 1, 'tb_participant', 175, '2021-01-25 18:08:43', 'I', 'Inclusão de paciente: 12345678'),
(11, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-01-25 18:08:43', 'I', 'Inclusão do registro referente ao paciente: 175 para o hospital: 1'),
(11, 1, 1, 'tb_participant', 176, '2021-01-25 18:13:43', 'I', 'Inclusão de paciente: 123456789'),
(11, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-01-25 18:13:43', 'I', 'Inclusão do registro referente ao paciente: 176 para o hospital: 1'),
(11, 1, 1, 'tb_participant', 177, '2021-01-25 18:17:34', 'I', 'Inclusão de paciente: 1234567890'),
(11, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-01-25 18:17:34', 'I', 'Inclusão do registro referente ao paciente: 177 para o hospital: 1'),
(11, 1, 2, 'tb_participant', 178, '2021-01-25 18:18:35', 'I', 'Inclusão de paciente: 1234567'),
(11, 1, 2, 'tb_assessmentquestionnaire', 0, '2021-01-25 18:18:35', 'I', 'Inclusão do registro referente ao paciente: 178 para o hospital: 2'),
(11, 1, 1, 'tb_participant', 179, '2021-01-25 18:20:40', 'I', 'Inclusão de paciente: 111'),
(11, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-01-25 18:20:40', 'I', 'Inclusão do registro referente ao paciente: 179 para o hospital: 1'),
(11, 1, 1, 'tb_formRecord', 239, '2021-01-26 22:55:52', 'I', 'Inclusão de Modulo para paciente: 88'),
(11, 1, 1, 'tb_questiongroupformrecord', 1130, '2021-01-26 22:55:52', 'I', 'Inclusão de Resposta da 117:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1131, '2021-01-26 22:55:52', 'I', 'Inclusão de Resposta da 120:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1132, '2021-01-26 22:55:52', 'I', 'Inclusão de Resposta da 150:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1133, '2021-01-26 22:55:52', 'I', 'Inclusão de Resposta da 154:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1134, '2021-01-26 22:55:52', 'I', 'Inclusão de Resposta da 168:2021-01-19'),
(11, 1, 1, 'tb_questiongroupformrecord', 1135, '2021-01-26 22:55:52', 'I', 'Inclusão de Resposta da 217:36'),
(11, 1, 1, 'tb_formRecord', 240, '2021-01-26 23:00:54', 'I', 'Inclusão de Modulo para paciente: 88'),
(11, 1, 1, 'tb_questiongroupformrecord', 1136, '2021-01-26 23:00:54', 'I', 'Inclusão de Resposta da 33:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1137, '2021-01-26 23:00:54', 'I', 'Inclusão de Resposta da 39:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1138, '2021-01-26 23:00:54', 'I', 'Inclusão de Resposta da 152:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1139, '2021-01-26 23:00:54', 'I', 'Inclusão de Resposta da 168:2020-04-22'),
(11, 1, 1, 'tb_questiongroupformrecord', 1140, '2021-01-26 23:00:54', 'I', 'Inclusão de Resposta da 183:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1141, '2021-01-26 23:00:54', 'I', 'Inclusão de Resposta da 197:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1142, '2021-01-26 23:00:54', 'I', 'Inclusão de Resposta da 199:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1143, '2021-01-26 23:00:54', 'I', 'Inclusão de Resposta da 216:Verbal - 2:9'),
(11, 1, 1, 'tb_formRecord', 241, '2021-01-26 23:19:55', 'I', 'Inclusão de Modulo para paciente: 88'),
(11, 1, 1, 'tb_questiongroupformrecord', 1144, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 83:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1145, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 119:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1146, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 156:123'),
(11, 1, 1, 'tb_questiongroupformrecord', 1147, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 168:2020-04-30'),
(11, 1, 1, 'tb_questiongroupformrecord', 1148, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 177:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1149, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 183:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1150, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 184:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1151, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 185:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1152, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 218:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1153, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 220:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1154, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 241:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1155, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 250:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1156, '2021-01-26 23:19:55', 'I', 'Inclusão de Resposta da 251:Não - 15:296'),
(11, 1, 1, 'tb_formRecord', 242, '2021-02-02 18:07:45', 'I', 'Inclusão de Modulo para paciente: 88'),
(11, 1, 1, 'tb_questiongroupformrecord', 1157, '2021-02-02 18:07:45', 'I', 'Inclusão de Resposta da 35:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1158, '2021-02-02 18:07:45', 'I', 'Inclusão de Resposta da 36:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1159, '2021-02-02 18:07:45', 'I', 'Inclusão de Resposta da 119:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1160, '2021-02-02 18:07:45', 'I', 'Inclusão de Resposta da 155:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1161, '2021-02-02 18:07:45', 'I', 'Inclusão de Resposta da 168:2021-02-03'),
(11, 1, 1, 'tb_formRecord', 243, '2021-02-02 18:10:41', 'I', 'Inclusão de Modulo para paciente: 88'),
(11, 1, 1, 'tb_questiongroupformrecord', 1162, '2021-02-02 18:10:41', 'I', 'Inclusão de Resposta da 168:2021-02-04'),
(11, 1, 1, 'tb_formRecord', 244, '2021-02-02 18:13:37', 'I', 'Inclusão de Modulo para paciente: 88'),
(11, 1, 1, 'tb_questiongroupformrecord', 1163, '2021-02-02 18:13:38', 'I', 'Inclusão de Resposta da 166:Brazil - 5:45'),
(11, 1, 1, 'tb_questiongroupformrecord', 1164, '2021-02-02 18:13:38', 'I', 'Inclusão de Resposta da 167:2021-02-15'),
(11, 1, 1, 'tb_questiongroupformrecord', 1165, '2021-02-02 18:13:38', 'I', 'Inclusão de Resposta da 242:HUGG'),
(11, 1, 1, 'tb_questiongroupformrecord', 1166, '2021-02-02 18:32:56', 'I', 'Inclusão de Resposta da 83:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1166, '2021-02-02 18:32:56', 'A', 'Exclusão da Resposta: 2020-04-30 para Inclusão de Resposta da 177:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1166, '2021-02-02 18:38:41', 'A', 'Exclusão da Resposta: 297 para Inclusão de Resposta da 83:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 10, '2021-02-02 18:43:52', 'A', 'Exclusão da Resposta: 300 para Inclusão de Resposta da 150:Não - 15:296'),
(3, 7, 1, 'tb_questiongroupformrecord', 1166, '2021-02-02 19:32:26', 'A', 'Exclusão da Resposta: 296 para Inclusão de Resposta da 83:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1167, '2021-02-02 19:46:06', 'I', 'Inclusão de Resposta da 47:false'),
(11, 1, 1, 'tb_questiongroupformrecord', 1168, '2021-02-02 19:46:06', 'I', 'Inclusão de Resposta da 49:true'),
(11, 1, 1, 'tb_questiongroupformrecord', 1169, '2021-02-02 19:46:53', 'I', 'Inclusão de Resposta da 31:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1170, '2021-02-02 19:46:53', 'I', 'Inclusão de Resposta da 32:Não - 15:296');
INSERT INTO `tb_notificationrecord` (`userID`, `profileID`, `hospitalUnitID`, `tableName`, `rowdID`, `changedOn`, `operation`, `log`) VALUES
(11, 1, 1, 'tb_questiongroupformrecord', 1171, '2021-02-02 21:36:21', 'I', 'Inclusão de Resposta da 155:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1172, '2021-02-02 21:36:21', 'I', 'Inclusão de Resposta da 217:37'),
(11, 1, 1, 'tb_participant', 180, '2021-02-05 23:53:11', 'I', 'Inclusão de paciente: 9876543210'),
(11, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-02-05 23:53:11', 'I', 'Inclusão do registro referente ao paciente: 180 para o hospital: 1'),
(11, 1, 1, 'tb_user', 12, '2021-02-08 21:07:18', 'I', 'Inclusão de dados do Usuario: teste@teste.com'),
(11, 1, 1, 'tb_user', 13, '2021-02-08 21:10:27', 'I', 'Inclusão de dados do Usuario: teste11@email.com'),
(11, 1, 1, 'tb_user', 14, '2021-02-08 21:13:05', 'I', 'Inclusão de dados do Usuario: user@email.com'),
(14, 7, 1, 'tb_questiongroupformrecord', 1173, '2021-02-08 21:41:50', 'I', 'Inclusão de Resposta da 177:Sim - 15:298'),
(14, 7, 1, 'tb_questiongroupformrecord', 1174, '2021-02-08 21:41:50', 'I', 'Inclusão de Resposta da 183:Não - 15:296'),
(14, 7, 1, 'tb_questiongroupformrecord', 1175, '2021-02-08 21:41:50', 'I', 'Inclusão de Resposta da 185:Sim - 15:298'),
(14, 7, 1, 'tb_questiongroupformrecord', 1176, '2021-02-08 21:41:50', 'I', 'Inclusão de Resposta da 197:Não - 15:296'),
(14, 7, 1, 'tb_questiongroupformrecord', 1177, '2021-02-08 21:41:50', 'I', 'Inclusão de Resposta da 217:37'),
(11, 1, 1, 'tb_questiongroupformrecord', 1178, '2021-02-08 21:42:56', 'I', 'Inclusão de Resposta da 47:true'),
(11, 1, 1, 'tb_questiongroupformrecord', 1179, '2021-02-08 21:42:56', 'I', 'Inclusão de Resposta da 49:true'),
(11, 1, 1, 'tb_questiongroupformrecord', 1180, '2021-02-08 21:42:56', 'I', 'Inclusão de Resposta da 64:false'),
(11, 1, 1, 'tb_questiongroupformrecord', 1181, '2021-02-08 21:43:40', 'I', 'Inclusão de Resposta da 149:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1182, '2021-02-08 21:43:40', 'I', 'Inclusão de Resposta da 151:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1183, '2021-02-08 21:43:40', 'I', 'Inclusão de Resposta da 153:Desconhecido - 16:301'),
(11, 1, 1, 'tb_questiongroupformrecord', 1184, '2021-02-08 21:43:40', 'I', 'Inclusão de Resposta da 154:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1185, '2021-02-08 21:43:40', 'I', 'Inclusão de Resposta da 218:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1186, '2021-02-08 21:43:40', 'I', 'Inclusão de Resposta da 220:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1187, '2021-02-08 21:46:51', 'I', 'Inclusão de Resposta da 54:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1188, '2021-02-08 21:46:51', 'I', 'Inclusão de Resposta da 55:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1189, '2021-02-08 21:46:51', 'I', 'Inclusão de Resposta da 59:Desconhecido - 6:262'),
(11, 1, 1, 'tb_questiongroupformrecord', 1190, '2021-02-08 21:46:51', 'I', 'Inclusão de Resposta da 63:true'),
(11, 1, 1, 'tb_questiongroupformrecord', 1191, '2021-02-08 21:46:51', 'I', 'Inclusão de Resposta da 64:true'),
(11, 1, 1, 'tb_user', 15, '2021-02-08 22:39:51', 'I', 'Inclusão de dados do Usuario: rodrigo@gmail.com'),
(15, 6, 1, 'tb_questiongroupformrecord', 1192, '2021-02-08 22:41:40', 'I', 'Inclusão de Resposta da 47:false'),
(15, 6, 1, 'tb_questiongroupformrecord', 1193, '2021-02-08 22:41:40', 'I', 'Inclusão de Resposta da 48:false'),
(15, 6, 1, 'tb_questiongroupformrecord', 1194, '2021-02-08 22:41:40', 'I', 'Inclusão de Resposta da 49:false'),
(15, 6, 1, 'tb_questiongroupformrecord', 1195, '2021-02-08 22:41:40', 'I', 'Inclusão de Resposta da 63:false'),
(15, 6, 1, 'tb_questiongroupformrecord', 1196, '2021-02-08 22:41:40', 'I', 'Inclusão de Resposta da 64:true'),
(15, 6, 1, 'tb_formRecord', 245, '2021-02-08 22:42:29', 'I', 'Inclusão de Modulo para paciente: 95'),
(15, 6, 1, 'tb_questiongroupformrecord', 1197, '2021-02-08 22:42:29', 'I', 'Inclusão de Resposta da 168:2020-04-25'),
(15, 6, 1, 'tb_questiongroupformrecord', 1198, '2021-02-08 22:42:29', 'I', 'Inclusão de Resposta da 217:37'),
(15, 6, 1, 'tb_questiongroupformrecord', 1199, '2021-02-08 22:42:29', 'I', 'Inclusão de Resposta da 218:Não - 15:296'),
(15, 6, 1, 'tb_questiongroupformrecord', 1200, '2021-02-08 22:42:29', 'I', 'Inclusão de Resposta da 220:Não - 15:296'),
(15, 6, 1, 'tb_participant', 181, '2021-02-08 22:42:55', 'I', 'Inclusão de paciente: 10001000'),
(15, 6, 1, 'tb_assessmentquestionnaire', 0, '2021-02-08 22:42:55', 'I', 'Inclusão do registro referente ao paciente: 181 para o hospital: 1'),
(11, 1, 1, 'tb_participant', 182, '2021-02-08 22:54:08', 'I', 'Inclusão de paciente: 12345678910'),
(11, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-02-08 22:54:08', 'I', 'Inclusão do registro referente ao paciente: 182 para o hospital: 1'),
(15, 6, 1, 'tb_questiongroupformrecord', 1201, '2021-02-08 22:55:44', 'I', 'Inclusão de Resposta da 203:Não - 15:296'),
(15, 6, 1, 'tb_questiongroupformrecord', 1202, '2021-02-08 22:55:44', 'I', 'Inclusão de Resposta da 204:Sim - 15:298'),
(11, 1, 1, 'tb_formRecord', 246, '2021-02-24 00:25:47', 'I', 'Inclusão de Modulo para paciente: 181'),
(11, 1, 1, 'tb_questiongroupformrecord', 1203, '2021-02-24 00:25:47', 'I', 'Inclusão de Resposta da 49:true'),
(11, 1, 1, 'tb_questiongroupformrecord', 1204, '2021-02-24 00:25:47', 'I', 'Inclusão de Resposta da 51:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1205, '2021-02-24 00:25:47', 'I', 'Inclusão de Resposta da 61:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1206, '2021-02-24 00:25:47', 'I', 'Inclusão de Resposta da 62:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1207, '2021-02-24 00:25:47', 'I', 'Inclusão de Resposta da 167:2021-02-22'),
(11, 1, 1, 'tb_questiongroupformrecord', 1208, '2021-02-24 00:25:47', 'I', 'Inclusão de Resposta da 202:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1209, '2021-02-24 00:25:47', 'I', 'Inclusão de Resposta da 203:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1210, '2021-02-24 00:27:32', 'I', 'Inclusão de Resposta da 129:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1211, '2021-02-24 00:27:32', 'I', 'Inclusão de Resposta da 134:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1212, '2021-02-24 00:27:32', 'I', 'Inclusão de Resposta da 137:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1213, '2021-02-24 00:27:32', 'I', 'Inclusão de Resposta da 140:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1214, '2021-02-24 00:27:32', 'I', 'Inclusão de Resposta da 209:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1215, '2021-02-24 00:27:32', 'I', 'Inclusão de Resposta da 214:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1216, '2021-02-24 00:27:32', 'I', 'Inclusão de Resposta da 252:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1217, '2021-02-24 00:27:32', 'I', 'Inclusão de Resposta da 253:Sim - 15:298'),
(11, 1, 1, 'tb_participant', 181, '2021-02-24 00:27:47', 'A', 'Alteração - Prontuario de: 10001000 para 10001001'),
(11, 1, 1, 'tb_formRecord', 247, '2021-03-02 18:01:51', 'I', 'Inclusão de Modulo para paciente: 89'),
(11, 1, 1, 'tb_questiongroupformrecord', 1218, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 53:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1219, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 54:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1220, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 95:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1221, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 130:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1222, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 152:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1223, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 153:Não - 16:300'),
(11, 1, 1, 'tb_questiongroupformrecord', 1224, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 154:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1225, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 167:2021-01-14'),
(11, 1, 1, 'tb_questiongroupformrecord', 1226, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 190:Dor - 2:7'),
(11, 1, 1, 'tb_questiongroupformrecord', 1227, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 207:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1228, '2021-03-02 18:01:51', 'I', 'Inclusão de Resposta da 211:Não - 15:296'),
(11, 1, 1, 'tb_formRecord', 248, '2021-03-02 18:02:43', 'I', 'Inclusão de Modulo para paciente: 89'),
(11, 1, 1, 'tb_questiongroupformrecord', 1229, '2021-03-02 18:02:43', 'I', 'Inclusão de Resposta da 167:2021-03-01'),
(11, 1, 1, 'tb_formRecord', 249, '2021-03-02 18:06:06', 'I', 'Inclusão de Modulo para paciente: 89'),
(11, 1, 1, 'tb_questiongroupformrecord', 1230, '2021-03-02 18:06:06', 'I', 'Inclusão de Resposta da 52:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1231, '2021-03-02 18:06:06', 'I', 'Inclusão de Resposta da 60:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1232, '2021-03-02 18:06:06', 'I', 'Inclusão de Resposta da 108:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1233, '2021-03-02 18:06:06', 'I', 'Inclusão de Resposta da 109:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1234, '2021-03-02 18:06:06', 'I', 'Inclusão de Resposta da 167:2020-12-24'),
(11, 1, 1, 'tb_formRecord', 250, '2021-03-02 20:22:43', 'I', 'Inclusão de Modulo para paciente: 89'),
(11, 1, 1, 'tb_questiongroupformrecord', 1235, '2021-03-02 20:22:43', 'I', 'Inclusão de Resposta da 28:Desconhecido - 15:297'),
(11, 1, 1, 'tb_questiongroupformrecord', 1236, '2021-03-02 20:22:43', 'I', 'Inclusão de Resposta da 168:2021-02-09'),
(11, 1, 1, 'tb_questiongroupformrecord', 1237, '2021-03-02 20:22:43', 'I', 'Inclusão de Resposta da 187:Desconhecido - 15:297'),
(11, 1, 1, 'tb_formRecord', 251, '2021-03-02 21:00:51', 'I', 'Inclusão de Modulo para paciente: 89'),
(11, 1, 1, 'tb_questiongroupformrecord', 1238, '2021-03-02 21:00:51', 'I', 'Inclusão de Resposta da 168:2020-03-16'),
(11, 1, 1, 'tb_formRecord', 252, '2021-03-02 21:05:07', 'I', 'Inclusão de Modulo para paciente: 89'),
(11, 1, 1, 'tb_questiongroupformrecord', 1239, '2021-03-02 21:05:07', 'I', 'Inclusão de Resposta da 168:2020-12-25'),
(11, 1, 1, 'tb_formRecord', 253, '2021-03-02 21:20:40', 'I', 'Inclusão de Modulo para paciente: 89'),
(11, 1, 1, 'tb_questiongroupformrecord', 1240, '2021-03-02 21:20:40', 'I', 'Inclusão de Resposta da 124:2021-03-01'),
(11, 1, 1, 'tb_formRecord', 254, '2021-03-04 00:49:55', 'I', 'Inclusão de Modulo para paciente: 89'),
(11, 1, 1, 'tb_questiongroupformrecord', 1241, '2021-03-04 00:49:55', 'I', 'Inclusão de Resposta da 48:false'),
(11, 1, 1, 'tb_questiongroupformrecord', 1242, '2021-03-04 00:49:55', 'I', 'Inclusão de Resposta da 63:false'),
(11, 1, 1, 'tb_questiongroupformrecord', 1243, '2021-03-04 00:49:55', 'I', 'Inclusão de Resposta da 109:Sim - 15:298'),
(11, 1, 1, 'tb_questiongroupformrecord', 1244, '2021-03-04 00:49:55', 'I', 'Inclusão de Resposta da 167:2021-03-02'),
(11, 1, 1, 'tb_formRecord', 255, '2021-03-04 00:51:33', 'I', 'Inclusão de Modulo para paciente: 89'),
(11, 1, 1, 'tb_questiongroupformrecord', 1245, '2021-03-04 00:51:33', 'I', 'Inclusão de Resposta da 168:2021-03-02'),
(11, 1, 1, 'tb_questiongroupformrecord', 1246, '2021-03-04 00:51:33', 'I', 'Inclusão de Resposta da 218:Não - 15:296'),
(11, 1, 1, 'tb_questiongroupformrecord', 1247, '2021-03-04 00:51:33', 'I', 'Inclusão de Resposta da 220:Não - 15:296'),
(11, 1, 1, 'tb_formRecord', 256, '2021-03-16 20:02:42', 'I', 'Inclusão de Modulo para paciente: 89'),
(11, 1, 1, 'tb_questiongroupformrecord', 1248, '2021-03-16 20:02:42', 'I', 'Inclusão de Resposta da 168:2021-03-15T18:00'),
(11, 1, 1, 'tb_user', 16, '2021-03-16 21:06:42', 'I', 'Inclusão de dados do Usuario: nome@email.com'),
(11, 1, 1, 'tb_user', 17, '2021-03-16 21:20:12', 'I', 'Inclusão de dados do Usuario: nome@email.com'),
(11, 1, 1, 'tb_user', 18, '2021-03-16 21:24:26', 'I', 'Inclusão de dados do Usuario: nome@email.com'),
(11, 1, 1, 'tb_participant', 183, '2021-04-30 21:21:02', 'I', 'Inclusão de paciente: 11223344'),
(11, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-04-30 21:21:02', 'I', 'Inclusão do registro referente ao paciente: 183 para o hospital: 1'),
(11, 1, 1, 'tb_user', 19, '2021-04-30 21:24:42', 'I', 'Inclusão de dados do Usuario: novousuario@email.com'),
(11, 1, 1, 'tb_formRecord', 257, '2021-04-30 21:30:23', 'I', 'Inclusão de Modulo para paciente: 88'),
(11, 1, 1, 'tb_questiongroupformrecord', 1249, '2021-04-30 21:30:23', 'I', 'Inclusão de Resposta da 168:2021-04-28T17:32'),
(11, 1, 1, 'tb_questiongroupformrecord', 1250, '2021-04-30 21:30:23', 'I', 'Inclusão de Resposta da 217:43'),
(11, 1, 1, 'tb_user', 20, '2021-10-13 00:24:24', 'I', 'Inclusão de dados do Usuario: maya@gmail.com'),
(20, 1, 1, 'tb_participant', 184, '2021-10-13 00:49:18', 'I', 'Inclusão de paciente: 1234355345'),
(20, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-10-13 00:49:18', 'I', 'Inclusão do registro referente ao paciente: 184 para o hospital: 1'),
(11, 1, 1, 'tb_user', 21, '2021-10-15 11:33:36', 'I', 'Inclusão de dados do Usuario: shrek@gmail.com'),
(11, 1, 1, 'tb_user', 22, '2021-10-15 11:34:34', 'I', 'Inclusão de dados do Usuario: mayamoraiss@gmail.com'),
(20, 1, 1, 'tb_participant', 185, '2021-11-16 22:39:33', 'I', 'Inclusão de paciente: 657567577'),
(20, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-11-16 22:39:33', 'I', 'Inclusão do registro referente ao paciente: 185 para o hospital: 1'),
(20, 1, 1, 'tb_participant', 185, '2021-11-17 21:38:10', 'A', 'Alteração - Prontuario de: 657567577 para 6575675778'),
(20, 1, 1, 'tb_participant', 186, '2021-11-27 06:05:37', 'I', 'Inclusão de paciente: 94564563'),
(20, 1, 1, 'tb_assessmentquestionnaire', 0, '2021-11-27 06:05:37', 'I', 'Inclusão do registro referente ao paciente: 186 para o hospital: 1'),
(20, 1, 1, 'tb_formRecord', 258, '2021-11-30 23:14:48', 'I', 'Inclusão de Modulo para paciente: 186'),
(20, 1, 1, 'tb_questiongroupformrecord', 1251, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 29:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1252, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 33:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1253, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 34:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1254, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 35:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1255, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 36:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1256, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 37:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1257, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 38:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1258, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 39:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1259, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 47:true'),
(20, 1, 1, 'tb_questiongroupformrecord', 1260, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 48:false'),
(20, 1, 1, 'tb_questiongroupformrecord', 1261, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 49:false'),
(20, 1, 1, 'tb_questiongroupformrecord', 1262, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 50:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1263, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 51:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1264, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 52:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1265, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 53:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1266, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 54:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1267, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 55:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1268, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 56:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1269, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 57:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1270, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 58:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1271, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 59:Sim-não toma antivirais - 6:263'),
(20, 1, 1, 'tb_questiongroupformrecord', 1272, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 60:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1273, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 62:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1274, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 63:false'),
(20, 1, 1, 'tb_questiongroupformrecord', 1275, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 64:false'),
(20, 1, 1, 'tb_questiongroupformrecord', 1276, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 82:Oral - 4:14'),
(20, 1, 1, 'tb_questiongroupformrecord', 1277, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 87:100'),
(20, 1, 1, 'tb_questiongroupformrecord', 1278, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 89:1-5 L/min - 8:271'),
(20, 1, 1, 'tb_questiongroupformrecord', 1279, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 90:Máscara com reservatório - 7:268'),
(20, 1, 1, 'tb_questiongroupformrecord', 1280, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 92:Canalizado - 14:289'),
(20, 1, 1, 'tb_questiongroupformrecord', 1281, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 93:2021-11-09T09:00'),
(20, 1, 1, 'tb_questiongroupformrecord', 1282, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 94:34'),
(20, 1, 1, 'tb_questiongroupformrecord', 1283, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 95:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1284, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 96:23'),
(20, 1, 1, 'tb_questiongroupformrecord', 1285, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 97:2021-11-10T12:00'),
(20, 1, 1, 'tb_questiongroupformrecord', 1286, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 98:234'),
(20, 1, 1, 'tb_questiongroupformrecord', 1287, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 100:antimalarium'),
(20, 1, 1, 'tb_questiongroupformrecord', 1288, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 107:1902-09-09T03:04'),
(20, 1, 1, 'tb_questiongroupformrecord', 1289, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 108:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1290, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 109:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1291, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 110:Não informado - 16:299'),
(20, 1, 1, 'tb_questiongroupformrecord', 1292, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 111:Feminino - 13:284'),
(20, 1, 1, 'tb_questiongroupformrecord', 1293, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 114:1'),
(20, 1, 1, 'tb_questiongroupformrecord', 1294, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 115:23'),
(20, 1, 1, 'tb_questiongroupformrecord', 1295, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 116:3'),
(20, 1, 1, 'tb_questiongroupformrecord', 1296, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 117:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1297, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 118:34'),
(20, 1, 1, 'tb_questiongroupformrecord', 1298, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 119:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1299, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 120:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1300, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 127:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1301, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 128:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1302, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 129:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1303, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 130:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1304, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 132:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1305, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 133:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1306, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 134:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1307, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 135:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1308, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 136:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1309, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 137:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1310, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 138:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1311, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 139:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1312, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 140:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1313, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 144:24'),
(20, 1, 1, 'tb_questiongroupformrecord', 1314, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 145:4'),
(20, 1, 1, 'tb_questiongroupformrecord', 1315, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 146:8'),
(20, 1, 1, 'tb_questiongroupformrecord', 1316, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 147:7'),
(20, 1, 1, 'tb_questiongroupformrecord', 1317, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 148:2'),
(20, 1, 1, 'tb_questiongroupformrecord', 1318, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 149:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1319, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 150:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1320, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 151:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1321, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 152:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1322, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 153:Sim - 16:302'),
(20, 1, 1, 'tb_questiongroupformrecord', 1323, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 154:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1324, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 155:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1325, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 156:3'),
(20, 1, 1, 'tb_questiongroupformrecord', 1326, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 157:56'),
(20, 1, 1, 'tb_questiongroupformrecord', 1327, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 158:123'),
(20, 1, 1, 'tb_questiongroupformrecord', 1328, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 159:4'),
(20, 1, 1, 'tb_questiongroupformrecord', 1329, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 160:2'),
(20, 1, 1, 'tb_questiongroupformrecord', 1330, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 161:4'),
(20, 1, 1, 'tb_questiongroupformrecord', 1331, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 162:3'),
(20, 1, 1, 'tb_questiongroupformrecord', 1332, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 163:34'),
(20, 1, 1, 'tb_questiongroupformrecord', 1333, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 164:3'),
(20, 1, 1, 'tb_questiongroupformrecord', 1334, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 165:45'),
(20, 1, 1, 'tb_questiongroupformrecord', 1335, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 166:Israel - 5:121'),
(20, 1, 1, 'tb_questiongroupformrecord', 1336, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 167:2021-11-29T08:00'),
(20, 1, 1, 'tb_questiongroupformrecord', 1337, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 169:3'),
(20, 1, 1, 'tb_questiongroupformrecord', 1338, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 170:4'),
(20, 1, 1, 'tb_questiongroupformrecord', 1339, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 171:6'),
(20, 1, 1, 'tb_questiongroupformrecord', 1340, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 172:34'),
(20, 1, 1, 'tb_questiongroupformrecord', 1341, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 174:3'),
(20, 1, 1, 'tb_questiongroupformrecord', 1342, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 189:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1343, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 190:Indiferente - 2:8'),
(20, 1, 1, 'tb_questiongroupformrecord', 1344, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 191:10'),
(20, 1, 1, 'tb_questiongroupformrecord', 1345, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 192:5'),
(20, 1, 1, 'tb_questiongroupformrecord', 1346, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 193:90'),
(20, 1, 1, 'tb_questiongroupformrecord', 1347, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 194:7'),
(20, 1, 1, 'tb_questiongroupformrecord', 1348, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 195:6'),
(20, 1, 1, 'tb_questiongroupformrecord', 1349, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 196:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1350, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 198:15'),
(20, 1, 1, 'tb_questiongroupformrecord', 1351, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 199:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1352, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 200:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1353, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 201:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1354, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 202:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1355, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 203:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1356, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 204:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1357, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 205:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1358, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 206:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1359, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 207:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1360, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 208:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1361, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 209:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1362, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 210:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1363, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 211:Não - 15:296'),
(20, 1, 1, 'tb_questiongroupformrecord', 1364, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 212:19'),
(20, 1, 1, 'tb_questiongroupformrecord', 1365, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 213:Desconhecido - 15:297'),
(20, 1, 1, 'tb_questiongroupformrecord', 1366, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 214:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1367, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 215:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1368, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 225:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1369, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 226:34'),
(20, 1, 1, 'tb_questiongroupformrecord', 1370, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 241:Sim - 15:298'),
(20, 1, 1, 'tb_questiongroupformrecord', 1371, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 242:Formulario de teste'),
(20, 1, 1, 'tb_questiongroupformrecord', 1372, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 245:67'),
(20, 1, 1, 'tb_questiongroupformrecord', 1373, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 246:3'),
(20, 1, 1, 'tb_questiongroupformrecord', 1374, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 247:3'),
(20, 1, 1, 'tb_questiongroupformrecord', 1375, '2021-11-30 23:14:48', 'I', 'Inclusão de Resposta da 248:45'),
...
(1, 'Critérios Clínicos para Inclusão', ''),
...
CREATE TABLE `vw_listype_covidcrfrapid` (
  ...
  `ontologyURI` varchar(500) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `vw_questiongroupform_covidcrfrapid` (
  `crfFormsID` int(10) DEFAULT NULL,
  `questionID` int(10) DEFAULT NULL,
  `questionOrder` int(10) DEFAULT NULL,
  `form_OntologyURI` varchar(500) DEFAULT NULL,
  `question_OntologyURI` varchar(500) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `vw_questionnaire_covidcrfrapid` (
  `questionnaireID` int(255) DEFAULT NULL,
  `description` varchar(255) DEFAULT NULL,
  `ontologyURI` varchar(500) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
